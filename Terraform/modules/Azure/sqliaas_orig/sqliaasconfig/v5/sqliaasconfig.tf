...
module "getbuildsecrets" {
    ...
    secretname      = ["SQLIaaSAdmin","JobListUpload","${lookup(var.addaccount, var.domain)}","${lookup(var.addcomputer, var.domain)}","SQL-CMDB-Management"]
}
...
resource "azurerm_virtual_machine_extension" "sqlconfig" {
    ...
    settings = <<SETTINGS
    {
      "fileUris": [
        ...
        "https://yourstgacc.blob.core.windows.net/scom/OMAgent.zip"
      ]
    }
SETTINGS
