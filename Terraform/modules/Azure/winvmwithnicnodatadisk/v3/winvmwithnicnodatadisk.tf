...
#}
module "getwindowsiaasadmin" {
  source      = "../../getkeyvaultsecret/v1"
  secretname  = ["WindowsIaaSAdmin"]
}
...
resource "azurerm_virtual_machine" "winvm" {
    ...
    os_profile {
        ...
        admin_username      = "${element(module.getwindowsiaasadmin.secretname,0)}"
        admin_password      = "${element(module.getwindowsiaasadmin.secretvalue,0)}"
    }
