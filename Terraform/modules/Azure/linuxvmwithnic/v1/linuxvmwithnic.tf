...
#    source = "../linuxnetworkinterface"
#    location      = "${var.location}"
#    resourcegroup = "${var.resourcegroup}"
#    subnetname    = "${var.subnetname}"
#    hostname      = "${format("AZUL${upper(var.env)}%06d", count.index + module.hostnumber.number)}"
#    linuxvmcount  = "${var.linuxvmcount}"
#} YOU CANNOT REFERENCE COUNT WITHIN A MODULE
...
resource "azurerm_virtual_machine" "linuxvm" {
    ...
    #network_interface_ids            = ["${element(module.networkinterface.linuxvmnic, count.index)}"]
    vm_size                          = "${var.vmsize}"
