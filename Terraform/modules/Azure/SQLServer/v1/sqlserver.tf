...
resource "azurerm_storage_account" "sqlsecuritysa" {
  name                     = "${lower(var.application)}${lower(var.env)}sqlsecurity"
...
resource "azurerm_template_deployment" "securityAlertPolicies" {
  ...
  "resources":[
    {
      ...
      "name": "[concat(parameters('sqlserver'),'/securityAlertPolicies')]",
      "properties": {
          "state": "Enabled",
...
resource "azurerm_template_deployment" "activedirectoryadmin" {
  ...
  "resources":[
    {
      ...
      "name": "[concat(parameters('sqlserver'),'/activeDirectory')]",
...
output "sqlserversap" {
  value = "${azurerm_template_deployment.securityAlertPolicies.name}"
}
