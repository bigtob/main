...
/* data "azurerm_key_vault_secret" "serviceaccountpassword" {
...
# these key-value pairs are passed into the ARM Template's `parameters` block
  parameters {
    "FailoverClusterName" = "cls${substr(var.application,0,6)}${var.env}sql"
    ...
    "domainAccountPassword" = "${data.azurerm_key_vault_secret.domainjoinpassword.value}"
...
output "FailoverClusterName" {
  value = "${lookup(azurerm_template_deployment.SQLIaaSWFC.outputs , "FailoverClusterName", "clsACTestDEVsql")}"
}
